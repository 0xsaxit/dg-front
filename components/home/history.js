          ...
          <div className="account-other-inner-container">
            <Fade bottom distance="20px" duration="600">
              ...
              <div style={{ marginLeft:'3px' }}>
                ...
                <p className="account-other-p">
                  <b className="account-hover">Deposits/Withdrawals</b> | <abbr className="account-hover" onClick= {() => this.handlePlay()}>Gameplay{' '}</abbr>
                </p> :
                <p className="account-other-p">
                  <abbr className="account-hover" onClick= {() => this.handleHistory()}>Deposits/Withdrawals</abbr> | <b className="account-hover">Gameplay{' '}</b>
                </p> }
              </div>
            </Fade>
            ...
            <div id='tx-box-history'>
              ...
              { data.length != 0 && this.state.isLoadingData == false ?
                <div>
                  <div className='dataTable' style={{height: 'calc(100vh - 280px)', padding: 0}}>
                          ...
                          if (row.betAmount) {
                            ...
                            return (
                              <Table.Row>
                                ...
                                <Table.Cell id="gameplay-timestamp">{timestamp}</Table.Cell>
                                <Table.Cell>
                            ...
                            if (row.status === 'Failed')
                              return (
                                <Table.Row style={{ background: '#fb9ca7' }}>
                                  ...
                                  <Table.Cell style={{ color: 'white' }} id="ready-timestamp">{timestamp}</Table.Cell>
                                  <Table.Cell>
                            ...
                            if (row.status === 'Ready')
                              return (
                                <Table.Row style={{background: '#8fe08f'}}>
                                  <Table.Cell style={{ paddingLeft: '20px' }}><img src={mana} style={{ width: '18px', paddingRight: '3px', verticalAlign: 'middle', marginTop: '-3px' }}/>{row.type}</Table.Cell>
                                  <Table.Cell style={{ color: 'white' }}>{amount} MANA</Table.Cell>
                                  <Table.Cell style={{ color: 'white', paddingLeft: '0px' }}>{row.status}</Table.Cell>
                                  <Table.Cell style={{ color: 'white' }} id="ready-timestamp">{timestamp}</Table.Cell>
                                  <Table.Cell>
                                    <ModalWithdraw isLink={1} tx={row.txid} showSpinner={this.showSpinner} hideSpinner={this.hideSpinner}/>
                                  </Table.Cell>
                            ...
                            return (
                              <Table.Row>
                                <Table.Cell style={{paddingLeft:'20px'}}><img src={mana} style={{ width: '18px', paddingRight: '3px', verticalAlign: 'middle', marginTop: '-3px' }}/>{row.type}</Table.Cell>
                                <Table.Cell>{amount} MANA</Table.Cell>
                                <Table.Cell className="table-status">{row.status}</Table.Cell>
                                <Table.Cell className="table-status2">{timestamp}</Table.Cell>
              ...
              : this.state.isLoadingData == false ? <p className="account-other-inner-p"> There is no transaction history for this account. </p> 
