  ...
  useEffect(() => {
    if (instances) {
      (async function () {
        ...
        const amountAdjusted = new BigNumber(minting_price).div(new BigNumber(10).pow(Global.CONSTANTS.TOKEN_DECIMALS)).toFixed(2);
              ...
              {!checkEnoughETHorICE() ? (
                <>
                  <span className={styles.dg_insufficient}>
                    {state.mintToken === 'ETH' ? Number(state.userBalances[2][3]).toFixed(3) : Math.trunc(Number(state.iceAmounts?.ICE_AVAILABLE_AMOUNT)*100)/100} {state.mintToken} Available
                    &nbsp;
                  </span>
                  <br />
                  (On Polygon)
                ...
                <>
                  <span className={styles.dg_available}>
                    {state.mintToken === 'ETH' ? Number(state.userBalances[2][3]).toFixed(3) : Math.trunc(Number(state.iceAmounts?.ICE_AVAILABLE_AMOUNT)*100)/100} {state.mintToken}{' '}
                    Available&nbsp;
